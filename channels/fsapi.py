""" Channels app FsApi request handler module. """
from uuid import UUID
from django.conf import settings
from django.http import Http404
from django.shortcuts import get_object_or_404
from django.utils import timezone
from channels.models import Client
from fsapi.handlers import RequestHandler, register_handlers


class VertoConfHandler(RequestHandler):
    """ Verto module configuration request handler. """

    def __init__(self):
        super(VertoConfHandler, self).__init__(
            key_value='verto.conf',
            section='configuration',
        )

    def process(self, request):
        """ Process verto configuration requests. """
        template = 'channels/verto.conf.xml'
        context = {
            'debug_level': 10,
...
class VertoAuthHandler(RequestHandler):
    """ Client auth request handler. """

    def __init__(self):
        super(VertoAuthHandler, self).__init__(
    ...
    def process(self, request):
        """ Process fsapi client auth requests. """
        try:
...
class VertoLoginEventHandler(RequestHandler):
    """ Handle verto client login events. """

    def __init__(self):
        super(VertoLoginEventHandler, self).__init__(
    ...
    def process(self, request):
        ...
        """
        template = 'channels/verto.event.txt'
...
class VertoDisconnectEventHandler(RequestHandler):
    """ Handle verto client disconnect events. """

    def __init__(self):
        super(VertoDisconnectEventHandler, self).__init__(
